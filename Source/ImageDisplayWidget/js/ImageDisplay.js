    ...
    resizeImage: function () {
        $('img[data-height-constraint]').each(function () {
            ...
            img.onload = function () {
                ...
                var maxWidth = $(this).parent().find('img').attr('data-width-constraint'); // Max width for the image
                var maxHeight = $(this).parent().find('img').attr('data-height-constraint');    // Max height for the image
                var ratio = 0;  // Used for aspect ratio
                var width = img.naturalWidth;    // Current image width
                var height = img.naturalHeight;  // Current image height

                // Check if the current width is larger than the max
                if (width > maxWidth) {
                    ratio = maxWidth / width;   // get ratio for scaling image
                    $(this).parent().find('img').css("width", maxWidth); // Set new width
                    $(this).parent().find('img').css("height", height * ratio);  // Scale height based on ratio
                    height = height * ratio;    // Reset height to match scaled image
                    width = width * ratio;    // Reset width to match scaled image
                }
                ...
                if (height > maxHeight) {
                    ratio = maxHeight / height; // get ratio for scaling image
                    $(this).parent().find('img').css("height", maxHeight);   // Set new height
                    $(this).parent().find('img').css("width", width * ratio);    // Scale width based on ratio
                    width = width * ratio;    // Reset width to match scaled image
                    height = height * ratio;    // Reset height to match scaled image
                }
            }
